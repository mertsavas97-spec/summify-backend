...
function mapYoutubeError(error, videoId) {
  ...
  if (
    ...
    /no transcripts|transcript.*not available|captions|subtitle|disabled/i.test(lower)
  ) {
  ...
  if (
    (name.includes('VideoUnavailable') || /no longer available/i.test(lower)) &&
    !/transcript|caption|subtitle/i.test(lower)
  ) {
...
function isTranscriptUnavailableError(error) {
  ...
  return (
    ...
    name.includes('Transcript') ||
    name.includes('Disabled') ||
    /transcript unavailable|no transcripts|empty transcript|too short|captions|subtitle/i.test(
      message,
    )
  );
}
...
async function tryExtractTranscript(
  ...
  try {
    ...
    if (mapped.code === 'errYoutubeTranscriptUnavailable') {
      return null;
    }
    throw mapped;
  }
...
async function extractYouTubeContent(rawUrl) {
  const { videoId, canonicalUrl, originalUrl } = validateYouTubeUrl(rawUrl);
  const domain = 'youtube.com';

  logEvent('youtube_extract_start', { domain, url: canonicalUrl, originalUrl });
  ...
  try {
    ...
    logEvent('youtube_transcript_unavailable', {
      ...
      videoId,
      durationSeconds,
      reason: 'no_captions_or_transcript_failed',
    });
    ...
    let mapped = error.code ? error : mapYoutubeError(error, videoId);
    ...
    if (mapped.code === 'errYoutubeTranscriptUnavailable') {
      try {
        ...
        const durationSeconds = getDurationSeconds(player.videoDetails);
        if (durationSeconds > MAX_DURATION_SECONDS) {
          mapped = Object.assign(new Error('Video exceeds maximum duration'), {
        ...
        mapped =
          fallbackError.code ? fallbackError : mapYoutubeError(fallbackError, videoId);
      }
    }
