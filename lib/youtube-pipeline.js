...
async function tryYouTubeTranscriptProviders({
  ...
  try {
    const rapid = await fetchTranscriptViaRapidApi(videoId, logEvent);
    if (rapid?.segments?.length) {
      transcript = {
        segments: rapid.segments,
        language: rapid.language,
        provider: 'rapidapi',
      };
    }
  } catch {
    // fall through to youtube-transcript package and other providers
  }
  ...
  const isRapidApi = transcript.provider === 'rapidapi';
  const isPackageProvider = transcript.provider === 'youtube-transcript-package';
  const isTranscriptMethod = isRapidApi || isPackageProvider;
  ...
  return {
    success: true,
    method: isTranscriptMethod ? 'transcript' : 'captions',
    text,
    language: transcript.language,
    segments: cleaned,
    extractionMethod: isTranscriptMethod ? 'transcript' : 'backend_youtube_transcript',
    transcriptSource: isRapidApi
      ? 'rapidapi'
      : isPackageProvider
        ? 'youtube-transcript-package'
        : 'captions',
