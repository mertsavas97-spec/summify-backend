...
async function findFirstAudioFileInDir(dir) {
  const entries = await fs.promises.readdir(dir, { withFileTypes: true });
  for (const ent of entries) {
    const fullPath = path.join(dir, ent.name);
    if (ent.isDirectory()) {
      const nested = await findFirstAudioFileInDir(fullPath);
...
async function readAudioFileFromTmpDir(tmpDir, methodLabel) {
  const filePath = await findFirstAudioFileInDir(tmpDir);
  if (!filePath) {
    const err = new Error(`${methodLabel}: no audio output file`);
...
function buildBundledYtDlpCliArgs(canonicalUrl, outTemplate, section = null) {
  const cookiePath = getCliCookiesPath();
  const args = [
    '--no-playlist',
    '--no-warnings',
    '-f',
    'ba/bestaudio/bestaudio[ext=m4a]/bestaudio',
    '-o',
    outTemplate,
    '--max-filesize',
...
async function downloadWithYtdlpNode(videoId, canonicalUrl) {
  ...
  try {
    const { args } = buildBundledYtDlpCliArgs(url, outTemplate);
    if (cookiePath) {
      logCookiesUsedForYtdlp();
    }
...
async function downloadAudioWithYtDlpNodeSection(canonicalUrl, section, videoId) {
  ...
  try {
    ...
    const { args } = buildBundledYtDlpCliArgs(canonicalUrl, outTemplate, section);
    if (cookiePath) logCookiesUsedForYtdlp();
