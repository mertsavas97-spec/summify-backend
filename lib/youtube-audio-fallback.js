...
async function downloadWithYtdlpNode(videoId, canonicalUrl) {
  ...
  try {
    ...
    if (cookieOpts.cookies) {
      builder = builder.cookies(cookieOpts.cookies);
      logCookiesUsedForYtdlp();
    }
    ...
    const audioFile = files.find(
      (f) => !f.endsWith('.part') && !f.endsWith('.ytdl') && !f.startsWith('.'),
    );
    if (!audioFile) {
      const err = new Error('ytdlp-nodejs: no audio output file');
      err.code = AUDIO_FALLBACK_ERROR;
      throw err;
    }
    ...
    const mimeType =
      ext === 'mp3' ? 'audio/mpeg' : ext === 'm4a' ? 'audio/mp4' : 'audio/mpeg';
