...
async function fetchTranscriptViaRapidApi(videoId, logEvent) {
  ...
  try {
    logEvent?.('transcript_rapidapi_attempt', { videoId });

    const response = await fetchWithTimeout(
      `https://youtube-transcript3.p.rapidapi.com/api/transcript?videoId=${encodeURIComponent(videoId)}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!data || !data.transcript || data.transcript.length === 0) {
      throw new Error('RapidAPI returned empty transcript');
    }
    ...
    const segments = cleanSegments(
      data.transcript.map((t, index) => ({
        text: t.text,
        offset: t.start ?? t.offset ?? index * 1000,
    ...
    logEvent?.('transcript_rapidapi_success', { videoId, chars: text.length });
    ...
    logEvent?.('transcript_rapidapi_failed', {
      videoId,
      reason: String(error?.message ?? error).slice(0, 160),
    });
    throw new Error(`RapidAPI transcript failed: ${error.message}`);
  }
}
